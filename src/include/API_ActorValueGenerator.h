...
namespace RE {
    // If there is ever some detail of general AV usage that needs access (GetDelay, SetDelay, etc) I will place them
    // here.
    ...
    inline RE::ActorValue GetActorValueIDFromName(std::string_view av_name) {
        // Please make sure that you are using a null terminated string view. Wouldn't want it to be completely
        // incorrect.
...
namespace AVG {
#define ACTOR_VALUE_CHANGE_PARAMS \
    RE::Actor *target, RE::Actor *cause, RE::ActorValue av, RE::ACTOR_VALUE_MODIFIER modifier, float before, float after
    ...
    namespace API {
        ...
        struct InterfaceVersion2 : public InterfaceVersion1 {
            ...
            /// <summary>
            /// Registers a function to fire after an ActorValue changes it's base or modifier values. Function requires
            /// parameters: <para>  RE::Actor* target: the target of the change</para> <para>	RE::Actor* cause: cause
            /// of the change (available only on damage)</para> <para>	RE::ActorValue av: The Changing actor
            /// value</para> <para>	RE::ACTOR_VALUE_MODIFIER modifier: The modifier impacted (kTotal is the base
            /// value)</para> <para>	float prev_value: The value before the change happened</para> <para>	float
            /// new_value: The value before the change happened</para> <para>	bool recursive: A check if this event is
            /// firing off within another event</para> <para>	*Use macro ACTOR_VALUE_CHANGE_PARAMS to set up
            /// parameters as above automatically </para>
            /// </summary>
        ...
        inline void* RequestInterface(Version version) {
            ...
            if (request_interface) {
                if (static unsigned int once = 0; once++) spdlog::info("Successful module and request, AVG");
            } else {
    ...
    struct ExtraValue {
        // It is in one's best interest to NEVER statically create these. Needless to say it wouldn't wait to see if AVG
        // exists, and would get none
        ...
        constexpr operator RE::ActorValue() { return _av; }
